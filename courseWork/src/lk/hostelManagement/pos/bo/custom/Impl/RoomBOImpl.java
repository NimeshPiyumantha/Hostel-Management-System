...
public class RoomBOImpl implements RoomBO {
    ...
    public ArrayList<RoomDTO> getAllRooms() throws SQLException, ClassNotFoundException {
        ...
        for (Room room : all) {
            allSRoom.add(new RoomDTO(room.getRoom_id(), room.getType(), room.getMonthly_rent(), room.getQty()));
        }
        return allSRoom;
    }
    ...
    public boolean saveRooms(RoomDTO dto) throws SQLException, ClassNotFoundException {
        return roomDAO.save(new Room(dto.getRoom_id(), dto.getType(), dto.getMonthly_rent(), dto.getQty()));
